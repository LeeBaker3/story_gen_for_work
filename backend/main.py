...
async def create_new_story(
    ...
    try:
        ...
        ai_story_input = story_input.dict()
        generated_content = await asyncio.to_thread(ai_services.generate_story_from_chatgpt, ai_story_input)
        ...
        if not ai_pages_data:
            error_logger.error(
                f"No pages generated by AI for input: {ai_story_input}")
            raise HTTPException(
                status_code=500, detail="AI failed to generate story pages.")

        # 2. Create initial story entry in the database
        db_story = crud.create_story_db_entry(
            db=db, story_data=story_input, user_id=current_user.id, title=story_title)
        app_logger.info(
            f"Story '{story_title}' (ID: {db_story.id}) created in DB for user {current_user.username}")

        # 3. Process each page: generate image, save page to DB
